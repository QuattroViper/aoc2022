package org.example;

import java.util.*;
import java.util.concurrent.atomic.AtomicInteger;
import java.util.stream.Collectors;

public class DayEight implements Day {
    @Override
    public String getData() {
        return rawData();
    }

    @Override
    public String process() {
        List<String> lines = List.of(getData().split("\n"));

        Forest forest = new Forest(lines);

        forest.printForestMap();
        return String.format("Trees visible : %s", forest.getVisibleTreeCount());

    }


    public static class Forest {

        private int forestY;
        private int forestX;

        private final List<Tree> trees = new ArrayList<>();

        public Forest(List<String> lines) {
            this.forestY = lines.size();
            this.forestX = lines.get(0).length();

            // Go down vertical
            for (int y = 0; y < forestY; y++) {

                // Go right horizontal
                for (int x = 0; x < forestX; x++) {
                    int treeHeight = Integer.parseInt(String.valueOf(lines.get(y).toCharArray()[x]));


                    int treeAbove = getHeightAtXandY(lines, y - 1, x);
                    int treeBelow = getHeightAtXandY(lines, y + 1, x);
                    int treeRight = getHeightAtXandY(lines, y, x + 1);
                    int treeLeft = getHeightAtXandY(lines, y, x - 1);

                    trees.add(new Tree(treeHeight, treeAbove, treeBelow, treeRight, treeLeft, x, y));
                }
            }
        }

        public void printForestMap() {
            String[][] treeMap = new String[forestX][forestY];

            for (Tree tree : this.trees) {
                treeMap[tree.x][tree.y] = tree.isTreeVisible() ? String.valueOf(tree.treeHeight) : String.valueOf(tree.treeHeight);
            }

            for (int y = 0; y < forestY; y++) {
                for (int x = 0; x < forestX; x++) {
                    System.out.print(treeMap[x][y]);
                }
                System.out.println("");
            }
        }

        public long getLocalVisibleTreeCount() {
            System.out.println("Trees Outside : " + trees.stream().filter(Tree::isTreeOutside).count());
            System.out.println("Trees Inside : " + trees.stream().filter(Tree::isTreeInside).count());
            System.out.println("Trees Inside and visible : " + trees.stream().filter(Tree::isTreeInsideAndVisible).count());

            return trees.stream().filter(Tree::isTreeVisible).count();
        }

        public long getVisibleTreeCount() {
            long v = 0;
            v = this.trees.stream().map(tree -> {
                        boolean treeIsEdge = tree.x == 0 || tree.y == 0 || tree.x == forestX - 1 || tree.y == forestY - 1;
                        // Is this an edge tree

                        List<Tree> treesInBothDirections = this.trees
                                .stream()
                                .filter(treeFilter -> !treeFilter.id.equals(tree.id)) // Ignore the tree itself
                                .filter(treeFilter -> treeFilter.x == tree.x || treeFilter.y == tree.y) // Get trees in up and down directions only
                                .toList();

                        // Check inside if trees are larger.
                        // Trees that is bigger or equal
                        boolean hasTreesAboveLarger = treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    // Trees Above this tree
                                    return treeInList.y < tree.y;
                                }).anyMatch(treeInList -> {
                                    // Trees that is bigger or equal
                                    return treeInList.treeHeight >= tree.treeHeight;
                                });

                        // Check inside if trees are larger.
                        boolean hasTreesBelowLarger = treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    return treeInList.y > tree.y;
                                }).anyMatch(treeInList -> {
                                    return treeInList.treeHeight >= tree.treeHeight;
                                });

                        // Trees that is bigger or equal
                        boolean hasTreesLeftLarger = treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    return treeInList.x < tree.x;
                                }).anyMatch(treeInList -> {
                                    // Trees that is bigger or equal
                                    return treeInList.treeHeight >= tree.treeHeight;
                                });

                        // Trees that is bigger or equal
                        boolean hasTreesRightLarger = treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    return treeInList.x > tree.x;
                                }).anyMatch(treeInList -> {
                                    // Trees that is bigger or equal
                                    return treeInList.treeHeight >= tree.treeHeight;
                                });

                        boolean isHidden = !treeIsEdge && hasTreesAboveLarger && hasTreesBelowLarger && hasTreesLeftLarger && hasTreesRightLarger;

                        return String.format("ID: %s, isEdge: %s, Above: %s, Below: %s, Left: %s, Right: %s, isHidden: %s", tree.id, treeIsEdge, hasTreesAboveLarger, hasTreesBelowLarger, hasTreesLeftLarger, hasTreesRightLarger, isHidden);
                    })
                    .filter(s -> s.endsWith("false"))
                    .count();

            return v;

        }

        public Long getHighestScenicScore() {
            return this.trees
                    .stream()
                    .filter(treeFilter -> !(treeFilter.x == 0 || treeFilter.y == 0 || treeFilter.x == forestX - 1 || treeFilter.y == forestY - 1)) // Don't calculate for edge trees
                    .map(tree -> {

                        List<Tree> treesInBothDirections = this.trees
                                .stream()
                                .filter(treeFilter -> !treeFilter.id.equals(tree.id)) // Ignore the tree itself
                                .filter(treeFilter -> treeFilter.x == tree.x || treeFilter.y == tree.y) // Get trees in up and down directions only
                                .toList();

                        Set<Tree> above = new TreeSet<>(new TreeXComp());
                        above.addAll(
                                treesInBothDirections
                                        .stream()
                                        .filter(treeInList -> {
                                            // Trees Above this tree
                                            return treeInList.getY() < tree.getY() && treeInList.getX() == tree.getX();
                                        })
//                                        .sorted()
                                        .toList()
                        );

                        int treesVisibleAbove = 1;
                        for (Tree t : treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    // Trees Above this tree
                                    return treeInList.getY() < tree.getY() && treeInList.getX() == tree.getX();
                                })
                                .sorted(Comparator.comparingLong(Tree::getY).reversed())
//                                .takeWhile(tt -> tt.getTreeHeight() < tree.getTreeHeight() || tt.isTreeOutside())
                                .toList()) {
                            if (t.getTreeHeight() >= tree.getTreeHeight() || t.isTreeOutside()) {
                                break;
                            } else {
                                treesVisibleAbove++;
                            }
                        }

                        int treesVisibleBelow = 1;
                        for (Tree t : treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    // Trees Above this tree
                                    return treeInList.getY() > tree.getY() && treeInList.getX() == tree.getX();
                                })
                                .sorted(Comparator.comparingLong(Tree::getY))
                                .toList()) {
                            if (t.getTreeHeight() >= tree.getTreeHeight() || t.isTreeOutside()) {
                                break;
                            } else {
                                treesVisibleBelow++;
                            }
                        }

                        long treesVisibleLeft = 1;
                        for (Tree t : treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    // Trees Above this tree
                                    return treeInList.getX() < tree.getX() && treeInList.getY() == tree.getY();
                                })
                                .sorted(Comparator.comparingLong(Tree::getX).reversed())
                                .toList()) {
                            if (t.getTreeHeight() >= tree.getTreeHeight() || t.isTreeOutside()) {
                                break;
                            } else {
                                treesVisibleLeft++;
                            }
                        }

                        long treesVisibleRight = 1;
                        for (Tree t : treesInBothDirections
                                .stream()
                                .filter(treeInList -> {
                                    // Trees Above this tree
                                    return treeInList.getX() > tree.getX() && treeInList.getY() == tree.getY();
                                })
                                .sorted(Comparator.comparingLong(Tree::getX))
                                .toList()) {
                            if (t.getTreeHeight() >= tree.getTreeHeight() || t.isTreeOutside()) {
                                break;
                            } else {
                                treesVisibleRight++;
                            }
                        }

                        return treesVisibleAbove * treesVisibleBelow * treesVisibleLeft * treesVisibleRight;
                    })
                    .max(Comparator.naturalOrder())
                    .orElse(0L);
        }

        public Integer getHeightAtXandY(List<String> lines, int y, int x) {
            // Get any line. If it's out of bounds, return -1, because tree is at end, and is always visible.
            try {
                String yLine = lines.get(y);

                Character xChar = yLine.toCharArray()[x];

                return Integer.parseInt(String.valueOf(xChar));

            } catch (IndexOutOfBoundsException | NumberFormatException e) {
                return -1;
            }

        }
    }

    public static class TreeXComp implements Comparator<Tree> {


        @Override
        public int compare(Tree o1, Tree o2) {
            return Long.compare(o1.x, o2.x);
        }

        @Override
        public Comparator<Tree> reversed() {
            return Comparator.super.reversed();
        }


    }

    public static class Tree {

        private String id;
        private int treeHeight;

        private int x;
        private int y;

        private int treeAboveHeight;
        private int treeBelowHeight;
        private int treeRightHeight;
        private int treeLeftHeight;

        public Tree(int treeHeight, int treeAboveHeight, int treeBelowHeight, int treeRightHeight, int treeLeftHeight, int x, int y) {
            this.id = String.format("%s%s", y, x);
            this.treeHeight = treeHeight;
            this.treeAboveHeight = treeAboveHeight;
            this.treeBelowHeight = treeBelowHeight;
            this.treeRightHeight = treeRightHeight;
            this.treeLeftHeight = treeLeftHeight;
            this.x = x;
            this.y = y;
        }

        public boolean isTreeVisible() {
            return (treeHeight > treeAboveHeight || treeHeight > treeBelowHeight || treeHeight > treeRightHeight || treeHeight > treeLeftHeight);
        }

        public boolean isTreeOutside() {
            return treeAboveHeight == -1 || treeBelowHeight == -1 || treeLeftHeight == -1 || treeRightHeight == -1;
        }

        public boolean isTreeInside() {
            return treeAboveHeight != -1 && treeBelowHeight != -1 && treeLeftHeight != -1 && treeRightHeight != -1;
        }

        public boolean isTreeInsideAndVisible() {
            return isTreeInside() && isTreeVisible();
        }

        public String getId() {
            return id;
        }

        public int getTreeHeight() {
            return treeHeight;
        }

        public int getX() {
            return x;
        }

        public int getY() {
            return y;
        }

        @Override
        public boolean equals(Object o) {
            if (this == o) return true;
            if (o == null || getClass() != o.getClass()) return false;
            Tree tree = (Tree) o;
            return treeHeight == tree.treeHeight && x == tree.x && y == tree.y && treeAboveHeight == tree.treeAboveHeight && treeBelowHeight == tree.treeBelowHeight && treeRightHeight == tree.treeRightHeight && treeLeftHeight == tree.treeLeftHeight && Objects.equals(id, tree.id);
        }

        @Override
        public int hashCode() {
            return Objects.hash(id, treeHeight, x, y, treeAboveHeight, treeBelowHeight, treeRightHeight, treeLeftHeight);
        }
    }

    @Override
    public String processMore() {
        List<String> lines = List.of(getData().split("\n"));

        Forest forest = new Forest(lines);

        forest.printForestMap();
        return String.format("Highest Scenic Score : %s", forest.getHighestScenicScore());
    }

    @Override
    public String rawData() {
        return """
                """;
    }

    @Override
    public String exampleRawData() {
        return """
                30373
                25512
                65332
                33549
                35390""";
    }
}
